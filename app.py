...
# ── 從 Streamlit Secrets 讀取帳號設定 ──────────────────────────────────────
credentials = {
    "usernames": {
        st.secrets["user1_username"]: {
            "name": st.secrets["user1_name"],
            "email": st.secrets["user1_email"],
            "password": st.secrets["user1_password"],
        },
        st.secrets["user2_username"]: {
            "name": st.secrets["user2_name"],
            "email": st.secrets["user2_email"],
            "password": st.secrets["user2_password"],
        },
        st.secrets["user3_username"]: {
            "name": st.secrets["user3_name"],
            "email": st.secrets["user3_email"],
            "password": st.secrets["user3_password"],
        },
    }
}
...
# ── 登入成功後顯示儀表板 ──────────────────────────────────────────────────
name = st.session_state.get("name", "使用者")
authenticator.logout(button_name=f"登出（{name}）", location="sidebar")
...
# ── 標題 ──────────────────────────────────────────────────────────────────
st.title("🛒 超市銷售儀表板")
st.caption(f"資料期間：{df['Date'].min().strftime('%Y/%m/%d')} ～ {df['Date'].max().strftime('%Y/%m/%d')}　｜　已篩選 {len(filtered):,} 筆 / 共 {len(df):,} 筆")
...
with col1:
    st.subheader("各分店營收")
    branch_rev = (
        filtered.groupby("Branch")["Total"].sum().reset_index()
        .rename(columns={"Total": "營收"})
        .sort_values("營收", ascending=False)
    )
    fig = px.bar(branch_rev, x="Branch", y="營收",
                 color="Branch", text_auto=".2s",
...
with col4:
    ...
    pay.columns = ["付款方式", "筆數"]
    fig = px.bar(pay, x="付款方式", y="筆數",
                 color="付款方式", text_auto=True,
                 color_discrete_sequence=px.colors.qualitative.Set3)
    fig.update_layout(showlegend=False, xaxis_title="", yaxis_title="筆數")
    st.plotly_chart(fig, use_container_width=True)
...
with col6:
    ...
    cat_rating = (
        filtered.groupby("Product line")["Rating"].mean().reset_index()
        .rename(columns={"Rating": "平均評分"})
        .sort_values("平均評分", ascending=True)
    )
    fig = px.bar(cat_rating, x="平均評分", y="Product line",
